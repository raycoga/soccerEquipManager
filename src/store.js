...
const reducerManager = (state = initialState, action) => {
    if (action.type === "AGREGAR_TITULAR") {
        return {
            ...
            jugadores: state.jugadores.filter(aux => aux.id !== action.jugador.id)
        }
    }
    if (action.type === "QUITAR_TITULAR") {
        return {
            ...state,
            titulares: state.titulares.filter(aux => aux.id !== action.jugador.id),
    ...
    if (action.type === "QUITAR_SUPLENTE") {
        return {
            ...state,
            suplentes: state.suplentes.filter(aux => aux.id !== action.jugador.id),
            jugadores: state.jugadores.concat(action.jugador)
...
/* Create store recibe como parametro otra funcion, que es la encargada de hacer los cambios en nuestra aplicacion, esta es la funcion reductora */
